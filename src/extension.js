...
class Imports {
  ...
  requiresPart (part, validOverrides = []) {
    const formattedPart = !part.startsWith('part')
      ? "part '" + part + "';"
      : part

    if (
      !this.includes(formattedPart) &&
      !this.hastAtLeastOneImport(validOverrides)
    ) {
      this.values.push(formattedPart)
    }
  }
}
...
class DataClassGenerator {
  ...
  generateDataClazzes () {
    ...
    for (let clazz of this.clazzes) {
      ...
      this.requiresImport('package:freezed_annotation/freezed_annotation.dart')

      const fileName = getDoc()
        .fileName.split(/[\\/]/)
        .pop()
        .replace('.dart', '')
      this.requiresPart(`${fileName}.freezed.dart`)
      this.requiresPart(`${fileName}.g.dart`)
  ...
  insertConstructor (clazz) {
    ...
    if (clazz.constr != null) {
      ...
      if (clazz.isWidget) constr += 'const '
    }

    constr += 'const factory ' + clazz.name + startBracket + '\n'
    ...
    for (let prop of clazz.properties) {
      ...
      if (!prop.isNullable) {
        ...
        if (hasDefault) {
          ...
          constr += `@JsonKey(name: '${prop.name}') required ${parameter},\n`
        } else {
          constr += `${parameter},\n`
        }
      } else {
    ...
    const stdConstrEnd = () => {
      constr +=
        endBracket +
        (clazz.isWidget ? ' : super(key: key);' : ` = _${clazz.name};`)
    }
  ...
  insertFromJson (clazz) {
    ...
    this.appendOrReplace(
      'fromJson',
      method,
      `factory ${clazz.name}.fromJson(Map<String, dynamic> json)`,
      clazz
    )
  }
